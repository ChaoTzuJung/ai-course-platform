...
// When Clerk is configured, wrap the root loader so auth state is available and
// the publishable key is forwarded to the client. Otherwise return null and the
// app renders without Clerk (dev-login mode).
...
(function() {
  try {
    var theme = localStorage.getItem('acp-theme');
    if (theme === 'dark') {
...
export default function App({ loaderData }: Route.ComponentProps) {
  ...
  // loaderData is Clerk's state object when configured, otherwise null.
  if (loaderData) {
    return <ClerkProvider loaderData={loaderData}>{content}</ClerkProvider>;
  }
  return content;
}
